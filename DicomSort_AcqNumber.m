...
%changed to acquisition number rather than series name...
...
for i = 3:length(dirInfo) %first 2 are '.' and '..'
    ...
    if isdir(filePath) == 0 && strncmp(dirInfo(i).name, 'IM_',3) == 1
        ...
        try
            ...
            if strcmp(scanNumber, scanNumberPrev) == 0 %Meaning we need a new folder because the scan changed
                count = count + 1;
                cd(SORTED)
                if scanNumber < 10
                ...
                mkdir(newFolderName)
                cd(newFolderName)
                %fprintf('Creating Scan Number %.2f\n', scanNumber)
            end
